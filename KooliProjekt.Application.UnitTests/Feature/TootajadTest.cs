...
{
    public class TootajadTest : ServiceTestBase
    {
        [Theory]
        [InlineData(0)]
        [InlineData(-10)]
        ...
        {
            // Arrange
            var query = new ListTootajaQuery { Page = page, PageSize = 5 };
            ...
            var handler = new ListTootajaQueryHandler(DbContext);
            await DbContext.Tootajad.AddAsync(tootaja);
        ...
        {
            // Arrange
            var query = new ListTootajaQuery { Page = 1, PageSize = page };
            var tootaja = new Tootaja
            {
                Nimi = "Toomas",
                Email = "toomas@test.ee",
        ...
        public async Task throws_if_dbcontext_is_null()
        {
            ...
            {
                new ListTootajaQueryHandler(null);
            });
        }
        ...
        {
            // Arrange
            var query = new ListTootajaQuery { Page = 1, PageSize = 5 };
            var tootaja = new Tootaja
            {
                Nimi = "Toomas",
                Email = "toomas@test.ee",
        ...
        {
            // Arrange
            var query = new ListTootajaQuery { Page = 101, PageSize = 10 };
            var tootaja = new Tootaja
            {
                Nimi = "Toomas",
                Email = "toomas@test.ee",
        ...
        public async Task Get_should_return_null_request_id_is_zero_or_less(int Id)
        {
            ...
            var handler = new GetTootajaQueryHandler(dbContext);
            var tootaja = new Tootaja
            {
                Nimi = "Toomas",
                Email = "toomas@test.ee",
        ...
        public async Task Get_throws_if_dbcontext_is_null()
        {
            Assert.Throws<ArgumentNullException>(() =>
            {
                new GetTootajaQueryHandler(null);
            });
            var query = new GetTootajaQuery { Id = 0 };
            var handler = new GetTootajaQueryHandler(DbContext);
            var result = await handler.Handle(query, CancellationToken.None);
        ...
        {
            // Arrange
            var query = new GetTootajaQuery { Id = 1 };
            var tootaja = new Tootaja
            {
                Nimi = "Toomas",
                Email = "toomas@test.ee",
        ...
        {
            // Arrange
            var query = new GetTootajaQuery { Id = 101 };
            var tootaja = new Tootaja
            {
                Nimi = "Toomas",
                Email = "toomas@test.ee",
        ...
        public void Save_should_throw_when_dbcontext_is_null()
        {
            Assert.Throws<ArgumentNullException>(() =>
            {
                new SaveTootajaCommandHandler(null);
            });
        }
        ...
        {
            // Arrange
            var request = (SaveTootajaCommand)null;
            var handler = new SaveTootajaCommandHandler(DbContext);
        ...
        {
            // Arrange
            var request = new SaveTootajaCommand { Id = -10 };
            var handler = new SaveTootajaCommandHandler(GetFaultyDbContext());
        ...
        {
            // Arrange
            var request = new SaveTootajaCommand { Id = 0, Nimi = "Toomas", Email = "toomas@test.ee", Ametikoht = "Testija" };
            var handler = new SaveTootajaCommandHandler(DbContext);

            // Act 
            var result = await handler.Handle(request, CancellationToken.None);
            var savedToDoList = await DbContext.Tootajad.SingleOrDefaultAsync(l => l.Id == 1);
        ...
        {
            // Arrange
            var listToAdd = new Tootaja { Id = 0, Nimi = "Toomas", Email = "toomas@test.ee", Ametikoht = "Testija" };
            var request = new SaveTootajaCommand { Id = 1, Nimi = "Toomas2", Email = "toomas2@test.ee", Ametikoht = "Testija2" };
            var handler = new SaveTootajaCommandHandler(DbContext);

            await DbContext.Tootajad.AddAsync(listToAdd);
            await DbContext.SaveChangesAsync();
        ...
        public async Task Save_should_return_error_if_list_does_not_exist()
        {
            // Arrange
            var listToAdd = new Tootaja { Id = 0, Nimi = "Toomas", Email = "toomas@test.ee", Ametikoht = "Testija" };
            var request = new SaveTootajaCommand { Id = 8, Nimi = "Toomas", Email = "toomas@test.ee", Ametikoht = "Testija" };
            var handler = new SaveTootajaCommandHandler(DbContext);
        ...
        {
            // Arrange
            var validator = new SaveTootajaCommandValidator(DbContext);
            var command = new SaveTootajaCommand { Id = 0, Nimi = nimi, Email = "toomas@test.ee", Ametikoht = "Testija" };
            // Act
            var result = validator.Validate(command);

            // Assert
            Assert.False(result.IsValid);
            Assert.Equal(nameof(SaveTootajaCommand.Nimi), result.Errors.First().PropertyName);
        }
        ...
        public void Delete_should_throw_when_dbcontext_is_null()
        {
            ...
            {
                new DeleteTootajaCommandHandler(db_context);
            });
        ...
        {
            // Arrange
            var request = (DeleteTootajaCommand)null;
            var handler = new DeleteTootajaCommandHandler(DbContext);
        ...
        {
            // Arrange
            var query = new DeleteTootajaCommand { Id = Id };
            var faultyDbContext = GetFaultyDbContext();
            var handler = new DeleteTootajaCommandHandler(faultyDbContext);
        ...
        {
            // Arrange
            var query = new DeleteTootajaCommand { Id = 1 };
            var handler = new DeleteTootajaCommandHandler(DbContext);
            ...
            // Act
            var result = await handler.Handle(query, CancellationToken.None);
            var listTest = await DbContext.Tootajad.FindAsync(query.Id);
        ...
        {
            // Arrange
            var query = new DeleteTootajaCommand { Id = 101 };
            var handler = new DeleteTootajaCommandHandler(DbContext);
